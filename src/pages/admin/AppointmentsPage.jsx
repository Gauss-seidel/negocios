              ...
              <tbody className="divide-y divide-gray-100">
                {appointments.map((appt) => (
                  <tr key={appt.id} className="transition-colors hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 sm:px-6 text-gray-600">{appt.service?.name || '—'}</td>
                    <td className="px-4 py-3 sm:px-6 text-gray-600 whitespace-nowrap">{formatDate(appt.date)}</td>
                    <td className="px-4 py-3 sm:px-6 text-gray-600">{formatTime(appt.start_time)}</td>
